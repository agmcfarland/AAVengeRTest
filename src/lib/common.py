...
class InputManager:
	...
	def __init__(self, working_dir, microb120_user, docker_image_name, aavenger_dir, source_mount, target_mount = '/data'):
		"""
...
class RunHandler:
	...
	def __init__(self, input_manager, run_id, random_seed_value, number_of_reads, n_cpus, config_preset, sample_sheet_path, run_tag):
		...
		self.stamped_run_id = make_stamped_run_id(input_manager.time_stamp, self.run_id, self.run_tag) # f'{input_manager.time_stamp}_{self.run_id}_{self.run_tag}'

		self.aavenger_output_dir = pjoin(self.input_manager.processed_dir, self.stamped_run_id)

		self.sample_sheet = pd.read_csv(sample_sheet_path, sep = '\t')


	def write_sample_sheet(self):
		self.sample_sheet.to_csv(pjoin(self.input_manager.raw_dir, 'sample_sheet.tsv'), sep = '\t', index = None)
	...
	def downsample_reads(self):
		...
		for read_type_ in self.read_files.keys():
			...
			self.read_files[read_type_]['downsampled'] = 'downsampled_' + self.read_files[read_type_]['raw']

			input_read = pjoin(self.input_manager.raw_dir, self.read_files[read_type_]['raw'])

			output_read = pjoin(self.input_manager.raw_dir, self.read_files[read_type_]['downsampled'])
	...
	def make_config_file(self):
		...
		with open(self.config_path, 'w') as outfile:
			...
			with open(pjoin(self.input_manager.aavenger_dir, 'config.yml'), 'r') as infile:
				...
				for l in infile:
					...
					if l.startswith('mode: integrase'):
						...
						outfile.write(f'softwareDir: {self.input_manager.aavenger_dir}\n')
						...
						outfile.write(f"demultiplex_anchorReadsFile: {pjoin(self.input_manager.raw_dir, self.read_files['R2']['downsampled'])}\n")

					elif l.startswith('demultiplex_adriftReadsFile'):
						outfile.write(f"demultiplex_adriftReadsFile: {pjoin(self.input_manager.raw_dir, self.read_files['R1']['downsampled'])}\n")

					elif l.startswith('demultiplex_index1ReadsFile'):
						outfile.write(f"demultiplex_index1ReadsFile: {pjoin(self.input_manager.raw_dir, self.read_files['I1']['downsampled'])}\n")

					elif l.startswith('demultiplex_sampleDataFile'):
						outfile.write(f"demultiplex_sampleDataFile: {pjoin(self.input_manager.raw_dir, 'sample_sheet.tsv')}\n")
					
					elif l.startswith('buildStdFragments_createMultiHitClusters'):
						outfile.write(f"buildStdFragments_createMultiHitClusters: True\n")
...
class AAVengeRDockerRunner:
	...
	def __init__(self, run_handler):
		...
		self.environment = {
			...
			"AAVENGER_CONFIG_PATH": pjoin(self.run_handler.config_path)  # Ensure these are capitalized
			}
...
class RunResultChecker:
	...
	def __init__(self, record_row):
		...
		self.stamped_run_id = make_stamped_run_id(self.time_stamp, self.run_id, self.run_tag) 

		self.run_output_dir = pjoin(self.working_dir, 'processed', self.time_stamp,  self.stamped_run_id)

		self.test_results_dir = pjoin(self.working_dir, 'test_results', self.time_stamp)

		self.record_identfier = record_row.to_frame().T

		self.record_identfier = self.record_identfier[['run_id', 'random_seed_value', 'number_of_reads', 'config_preset', 'run_tag', 'time_stamp', 'aavenger_version']]

		self.sites_feature_to_test = [
			'file_detected',
			'shape_rows', 'shape_columns',
			'n_subject', 'n_sample', 'n_refGenome', 'n_posid', 'n_vector', 'n_repeat_name',
			'n_repeat_class', 'n_nearestGene', 'sum_sonicLengths', 'sum_reads', 'sum_nRepsObs', 
			'sum_inGene', 'sum_inExon'
			 ]
	...
	def _read_in_sites(self):
		try:
			self.sites = pd.read_excel(pjoin(self.run_output_dir, 'annotateRepeats', 'sites.xlsx'))
	...
	def characterize_sites_features(self):
		...
		if self.sites_exist:
			self.sites['nRepsObs'] = self.sites['nRepsObs'].fillna(0) # nRepsObs is blank for dual detections
			...
			for feature_ in ['subject', 'sample', 'refGenome', 'posid', 'vector', 'repeat_name', 'repeat_class', 'nearestGene']:
				self.site_features[f'n_{feature_}'] = [len(self.sites[feature_].unique())]
	...
	def make_sites_df(self):
		...
		df_site_results = pd.concat([self.record_identfier.reset_index(drop = True), pd.DataFrame(self.site_features).reset_index(drop = True)], axis = 1)
...
class CompareResults:
	...
	def __init__(self, test_results_path, expected_results_path):
		...
		self.identifier_columns = ['run_id', 'random_seed_value', 'number_of_reads', 'config_preset', 'run_tag']#, 'time_stamp', 'aavenger_version']
		self.results = {
		'sites': {'raw': '', 'all': '', 'only_different': ''}
		}
	...
	def compare_sites(self):
		...
		df_expected = pd.read_csv(pjoin(self.expected_results_path, 'site_features.csv'))

		self.results['sites']['raw'] = self.merge_expected_actual_sites(df_actual = df_actual, df_expected = df_expected, identifier_columns = self.identifier_columns)
		...
		df_difference = self.merge_expected_actual_sites(df_actual = df_actual, df_expected = df_expected, identifier_columns = self.identifier_columns)

		feature_columns = [i for i in df_expected.columns.tolist() if i not in self.identifier_columns]

		df_results = self.build_compare_table(df_difference = df_difference, feature_columns = feature_columns, identifier_columns = self.identifier_columns)

		self.results['sites']['all'] = df_results

		self.results['sites']['only_different'] = df_results[df_results['difference'] != 0]
	...
	def build_compare_table(self, df_difference, feature_columns, identifier_columns):
		...
		for _, result_pair in df_difference.iterrows():
			...
			for feat_ in feature_columns:
				...
				result = {'feature': [feat_], 'actual': [result_pair[f"{feat_}_actual"]], 'expected': [result_pair[f"{feat_}_expected"]]}
			...
			df_identifier['difference'] = df_identifier['expected'] - df_identifier['actual']
	...
	def merge_expected_actual_sites(self, df_actual, df_expected, identifier_columns):
		...
		identifier_columns: list of columns with data that should be exactly the same for both expected and actual tables.
		"""
		return df_actual.merge(df_expected, on = identifier_columns, how = 'outer', suffixes = ('_actual', '_expected'))


	def write_comparisons(self):
		for k, v in self.results.items():
			v['raw'].to_csv(pjoin(self.test_results_path, f'{k}_raw_comparisons.csv'), index = None)
			v['all'].to_csv(pjoin(self.test_results_path, f'{k}_all_comparisons.csv'), index = None)
			v['only_different'].to_csv(pjoin(self.test_results_path, f'{k}_difference_comparisons.csv'), index = None)
