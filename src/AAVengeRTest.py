...
from lib.common import InputManager, RunHandler, AAVengeRDockerRunner, RunResultChecker, CompareResults
...
def main():
	...
	with Logger(
		...
		log_file = 'AAVengerTest') as logger:
		...
		global_input_parameters.load_test_data_sheet(test_data_sheet_path = args.test_data_sheet)
		...
		for _, run_ in global_input_parameters.df_test_data.iterrows():
			...
			docker_client = AAVengeRDockerRunner(
				run_handler = test_run_handler
				)
		...
		for _, record_row_ in df_record.iterrows():
			...
			run_result = RunResultChecker(
				record_row = record_row_
				)
			...
			df_site_results = pd.concat([df_site_results, run_result.make_sites_df()], axis = 0)
			
		df_site_results.to_csv(pjoin(run_result.test_results_dir, 'site_features.csv'), index = None)
		...
		if args.expected_output_dir != '':
			logger.info(f"Comparining run results to expected values")
