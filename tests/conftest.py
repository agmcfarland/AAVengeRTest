...
def mocked_input_manager(scope = 'session', autouse = True):
	...
	mock_input_manager.processed_dir =  pjoin(mock_input_manager.working_dir, 'processed')
...
def example_run_handler(mocked_input_manager, scope = 'session', autouse = True):
	handler = RunHandler(
		...
		sample_sheet_path = pathlib.Path(__file__).parent / 'data' / 'CompletedSampleSheet.tsv',
		n_cpus = 8,
...
def example_input_test_sheet(scope = 'session', autouse = True):
	return pd.DataFrame(
		...
		'sample_sheet_path': ['/path/to/sample_sheet1.tsv', '/path/to/sample_sheet2.tsv'],
		'n_CPUs': [8, 8],
...
@pytest.fixture
def example_run_handler_read_write(example_input_manager_read_write, scope = 'session', autouse = True):
	"""
	Real file structure set up
	"""
	handler = RunHandler(
