...
@pytest.fixture
def example_compare_results(project_test_data_directory, setup_temp_dir, scope = 'session', autouse = True):
...
def test_sites_comparison(example_compare_results, project_test_data_directory):
	...
	df_mock_actual = pd.read_csv(pjoin(project_test_data_directory, 'ete_expected_site_features.csv'))

	df_mock_expected = pd.read_csv(pjoin(project_test_data_directory, 'ete_expected_site_features_differing.csv'))
...
def test_write_comparisons(example_compare_results, project_test_data_directory):
	...
	with patch('pandas.DataFrame.to_csv') as mock_to_csv:
		...
		expected_calls = [
		(pjoin(project_test_data_directory, 'sites_raw_comparisons.csv'), {'index': None}),
		(pjoin(project_test_data_directory, 'sites_all_comparisons.csv'), {'index': None}),
		(pjoin(project_test_data_directory, 'sites_difference_comparisons.csv'), {'index': None})
		]

		mock_to_csv.assert_has_calls([call(args, **kwargs) for args, kwargs in expected_calls])
