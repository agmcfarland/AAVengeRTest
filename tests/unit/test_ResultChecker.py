...
@pytest.fixture
def example_run_results(
    project_test_data_directory, setup_temp_dir, scope="session", autouse=True
):
    record_row = pd.read_csv(
        pjoin(project_test_data_directory, "ete_correct_input_sheet_record.csv")
    )
    record_row = record_row.iloc[0, :]

    run_results = RunResultChecker(record_row=record_row)
    return run_results
...
def test_read_in_sites(example_run_results, project_test_data_directory):
    ...
    test_df = pd.read_excel(
        pjoin(project_test_data_directory, "ete_correct_sites.xlsx")
    )
    ...
    assert str(
        type(example_run_results.sites) == "<class 'pandas.core.frame.DataFrame'>"
    )
...
def test_characterize_site_features(example_run_results, project_test_data_directory):
    ...
    assert example_run_results.site_features == {
        "file_detected": [True],
        "shape_rows": [40],
        "shape_columns": [30],
        "n_subject": [1],
