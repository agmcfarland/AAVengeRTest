...
@patch("os.system")
def test_scp_pull_from_microb120(
    mock_os_system, mocked_input_manager, example_run_handler
):
    """
    ...
    expected_calls = [
        (
            call(
                "scp test_user@microb120.med.upenn.edu:/media/sequencing/Illumina/test_run/Data/Intensities/BaseCalls/Undetermined_S0_R1_001.fastq.gz test_working_dir/raw"
            )
        ),
        (
            call(
                "scp test_user@microb120.med.upenn.edu:/media/sequencing/Illumina/test_run/Data/Intensities/BaseCalls/Undetermined_S0_R2_001.fastq.gz test_working_dir/raw"
            )
        ),
        (
            call(
                "scp test_user@microb120.med.upenn.edu:/media/sequencing/Illumina/test_run/Data/Intensities/BaseCalls/Undetermined_S0_I1_001.fastq.gz test_working_dir/raw"
            )
        ),
    ]
...
def test_reformat_reads(mock_os_system, mocked_input_manager, example_run_handler):
    ...
    expected_calls = [
        (
            call(
                "reformat.sh in=test_working_dir/raw/Undetermined_S0_R1_001.fastq.gz out=test_working_dir/raw/downsampled_Undetermined_S0_R1_001.fastq.gz reads=1000 ow=t sampleseed=42"
            )
        ),
        (
            call(
                "reformat.sh in=test_working_dir/raw/Undetermined_S0_R2_001.fastq.gz out=test_working_dir/raw/downsampled_Undetermined_S0_R2_001.fastq.gz reads=1000 ow=t sampleseed=42"
            )
        ),
        (
            call(
                "reformat.sh in=test_working_dir/raw/Undetermined_S0_I1_001.fastq.gz out=test_working_dir/raw/downsampled_Undetermined_S0_I1_001.fastq.gz reads=1000 ow=t sampleseed=42"
            )
        ),
    ]
...
def test_write_sample_sheet(example_run_handler_read_write):
    ...
    assert os.path.exists(
        pjoin(example_run_handler_read_write.input_manager.raw_dir, "sample_sheet.tsv")
    )


def test_make_config_file_integrase(
    example_run_handler_read_write, project_test_data_directory
):
    """
    pytest -sv tests/unit/test_run_handler.py::test_make_config_file_integrase
    """

    example_run_handler_read_write.input_manager.aavenger_dir = (
        project_test_data_directory
    )

    # This will happen after

    example_run_handler_read_write.read_files["R1"][
        "downsampled"
    ] = "Undetermined_S0_R1_001.fastq.gz"

    example_run_handler_read_write.read_files["R2"][
        "downsampled"
    ] = "Undetermined_S0_R2_001.fastq.gz"

    example_run_handler_read_write.read_files["I1"][
        "downsampled"
    ...
    with open(example_run_handler_read_write.config_path, "r") as infile:
        for l in infile:
            ...
            if l.startswith("outputDir"):
                assert (
                    l
                    == f"outputDir: {example_run_handler_read_write.aavenger_output_dir}\n"
                )

            if l.startswith("softwareDir"):
                assert (
                    l
                    == f"softwareDir: {example_run_handler_read_write.input_manager.aavenger_dir}\n"
                )

            if l.startswith("sequencingRunID"):
                assert (
                    l
                    == f"sequencingRunID: {example_run_handler_read_write.stamped_run_id}\n"
                )

            if l.startswith("demultiplex_anchorReadsFile"):
                assert (
                    l
                    == f"demultiplex_anchorReadsFile: {pjoin(example_run_handler_read_write.input_manager.raw_dir, example_run_handler_read_write.read_files['R2']['downsampled'])}\n"
                )

            if l.startswith("demultiplex_adriftReadsFile"):
                assert (
                    l
                    == f"demultiplex_adriftReadsFile: {pjoin(example_run_handler_read_write.input_manager.raw_dir, example_run_handler_read_write.read_files['R1']['downsampled'])}\n"
                )

            if l.startswith("demultiplex_index1ReadsFile"):
                assert (
                    l
                    == f"demultiplex_index1ReadsFile: {pjoin(example_run_handler_read_write.input_manager.raw_dir, example_run_handler_read_write.read_files['I1']['downsampled'])}\n"
                )

            if l.startswith("demultiplex_sampleDataFile"):
                assert (
                    l
                    == f"demultiplex_sampleDataFile: {pjoin(example_run_handler_read_write.input_manager.raw_dir, 'sample_sheet.tsv')}\n"
                )
